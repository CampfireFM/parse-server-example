...
const resetTop20CloutPoints = () => {
    return new Promise((resolve, reject) => {
        ...
        query.greaterThanOrEqualTo('createdAt', new Date(new Date().getTime() - 7 * 24 * 3600 * 1000));
...
// New resetFeaturedAnswers - by top20 and featuredAnswerIds
const resetFeaturedAnswers = () => {
    return new Promise((resolve, reject) => {
        const Defaults = Parse.Object.extend('Defaults');
        const query = new Parse.Query(Defaults);
        query.first({useMasterKey: true})
            .then(defaultSettings => {
                const featuredAnswerIds = defaultSettings.get('featuredAnswers');
                redisClient.lrange('top20AnswerIds', 0, -1, (err, replies) => {
                    if (err) {
                        console.log(err);
                        reject(err);
                    } else {
                        console.log(replies);
                        let top20AnswerIds = replies;
                        top20AnswerIds = top20AnswerIds.filter(id => featuredAnswerIds.indexOf(id) == -1);
                        // Let's sort featured answers
                        let newFeaturedAnswerIds = [];
                        let index = 0;

                        featuredAnswerIds.forEach(featuredAnswerId => {
                            newFeaturedAnswerIds.push(featuredAnswerId);
                            newFeaturedAnswerIds.push(top20AnswerIds[index]);
                            index++;
                        });

                        const multi = redisClient.multi();
                        multi.del('featuredAnswers');
                        newFeaturedAnswerIds.forEach(answer => {
                            multi.rpush('featuredAnswers', answer);
                        });
                        multi.exec((err, res) => {
                            if (err) {
